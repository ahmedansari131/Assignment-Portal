...
const NormalPlagiarisedTable = (props) => {
  const { data } = props;
            ...
            <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="divide-y divide-gray-500/20 bg-gray-800/30">
                  {data?.map((item, index) => (
                    <tr key={item.plagiarism}>
                      <td className="px-4 py-4 text-sm font-medium ">
                        <div>
                          <h2 className="text-gray-800 dark:text-white flex items-center gap-4">
                            {item.student_one.split(".")[0][0].toUpperCase() +
                              item.student_one.split(".")[0].substring(1) +
                              " " +
                              item.student_one.split(".")[1][0].toUpperCase() +
                              item.student_one
                                .split(".")[1]
                                .substring(1)
                                .replace("@vit", "")}
                      ...
                      <td className="px-4 py-4 text-sm font-medium">
                        <div>
                          <h2 className="text-gray-800 dark:text-white flex items-center gap-4">
                            {item.student_two.split(".")[0][0].toUpperCase() +
                              item.student_two.split(".")[0].substring(1) +
                              " " +
                              item.student_two.split(".")[1][0].toUpperCase() +
                              item.student_two
                                .split(".")[1]
                                .substring(1)
                                .replace("@vit", "")}
...
export default NormalPlagiarisedTable;
